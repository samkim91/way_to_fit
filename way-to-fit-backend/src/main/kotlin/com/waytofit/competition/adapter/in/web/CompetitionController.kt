...
class CompetitionController(
    ...
    fun createCompetition(
        ...
        return ApiResponse.success(CompetitionResponse.fromDomain(competition, Instant.now(clock)))
    }
    ...
    fun getMyCompetitions(
        @CurrentUserId userId: UUID,
        @RequestParam(required = false) lifecycles: List<CompetitionLifecycle>?,
        pageable: Pageable
    ): ApiResponse<Page<CompetitionResponse>> {
        val now = Instant.now(clock)
        val competitions = competitionQueryUseCase.getMyCompetitions(userId, lifecycles, pageable)
        return ApiResponse.success(competitions.map { CompetitionResponse.fromDomain(it, now) })
    }
    ...
    fun getCompetition(
        ...
        val competition = competitionQueryUseCase.getCompetition(id, userId)
        return ApiResponse.success(CompetitionResponse.fromDomain(competition, Instant.now(clock)))
    }
